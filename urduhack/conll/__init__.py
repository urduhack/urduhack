...
whose details are given below:
    ...
    1. ID: ID represents the word/token index in the sentence
    2. FORM: Word/token form or punctuation symbol used in the sentence
    3. LEMMA: Root/stem of the word
    4. UPOS: Universal Part-of-Speech tag
    5. XPOS: Language specific part-of-speed tag. underscore if not available
    6. FEATS: List of morphological features from the universal features
        inventory or from a defined language specific extension
    7. HEAD: Head of the current word which is wither the value of ID or zero.
    8. DEPREL: Universal dependencies relation to the HEAD (root if HEAD=0)
        or a defined language specific subtype of one.
    9. DEPS: Enhanced dependency graph in the form of a list of head-deprel pairs
    10. MISC: Any other annotation apart from the above mentioned fields


Examples:
    >>> from urduhack.conll import CoNLL
    >>> conl_reader = CoNLL()
    >>> data = conl_reader.load_file(file_name="urdu_data.conll")
    >>> print(data[0][0]['text'])
